...
interface PendingRuleEditRequest {
  _id: string;
  accountId: string;
  subAccountName: string;
  marketTypeId: string;
  requestedAt: string;
  scheduledEditDate: string;
  editWindowEndDate: string;
  status: 'pending' | 'editable' | 'canceled' | 'completed';
}
...
export default function AddTradingRulesPage() {
  ...
  const [pendingRuleEditRequests, setPendingRuleEditRequests] = useState<PendingRuleEditRequest[]>([]);
  ...
  useEffect(() => {
    const fetchPendingRuleEditRequests = async () => {
      setIsLoadingPending(true);
      try {
        const response: Response = await TradingRulesService.getPendingRuleEditRequests();
        if (response.statusCode === 200 && response.success) {
          setPendingRuleEditRequests(Array.isArray(response.data) ? response.data : []);
          if (response.data.length === 0) {
            setResponseMessage({ text: "No pending rule edit requests", type: "info" });
            setShowSnackbar(true);
          }
        } else {
          setResponseMessage({ text: response.message || "Failed to fetch pending rule edit requests", type: "error" });
          setShowSnackbar(true);
        }
      } catch (error: any) {
        console.error("Error fetching pending rule edit requests:", error);
        setResponseMessage({ text: error.message || "Failed to fetch pending rule edit requests", type: "error" });
        setShowSnackbar(true);
      } finally {
        setIsLoadingPending(false);
      }
    };
    fetchPendingRuleEditRequests();
  }, []);

  const handleSubBrokerSelection = async (account: SubBrokerAccount) => {
    if (!account.hasProxy) {
      setResponseMessage({
    ...
    try {
      const response = await TradingRulesService.getTradingRules(account._id);
      if (response.statusCode === 200 && response.success) {
        const fetchedRules = response.data || { cash: [], future: [], option: [] };
        const initializeRules = (existing: RuleValue[]) => {
          return rules.map((rule) => {
            const existingRule = existing.find((r: RuleValue) => r.key === rule.key);
            return {
              key: rule.key,
        ...
        setTradingRules({
          cash: initializeRules(fetchedRules.cash),
          future: initializeRules(fetchedRules.future),
          option: initializeRules(fetchedRules.option),
        });
      } else {
        setResponseMessage({ text: response.message || "Failed to fetch trading rules", type: "error" });
        setShowSnackbar(true);
      }
    } catch (error: any) {
      console.error("Error fetching trading rules:", error);
      setResponseMessage({ text: error.message || "Failed to fetch trading rules", type: "error" });
      setShowSnackbar(true);
    }
  };
  ...
  const handleCancelRuleEditRequest = async (requestId: string) => {
    if (isSubmitting) return;
    setIsSubmitting(true);
    try {
      const response: Response = await TradingRulesService.cancelRuleEditRequest(requestId);
      if (response.statusCode === 200 && response.success) {
        setResponseMessage({ text: "Rule edit request canceled successfully", type: "success" });
        setShowSnackbar(true);
        const pendingResponse = await TradingRulesService.getPendingRuleEditRequests();
        if (pendingResponse.statusCode === 200 && pendingResponse.success) {
          setPendingRuleEditRequests(Array.isArray(pendingResponse.data) ? pendingResponse.data : []);
        }
      } else {
        setResponseMessage({ text: response.message || "Failed to cancel rule edit request", type: "error" });
        setShowSnackbar(true);
      }
    } catch (error: any) {
      console.error("Error canceling rule edit request:", error);
      setResponseMessage({ text: error.message || "Failed to cancel rule edit request", type: "error" });
      setShowSnackbar(true);
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isMainApiVerified || !isSubApiVerified) {
      setResponseMessage({
        text: "Please verify both Main and Sub-Account API keys",
        type: "error",
      });
      setShowSnackbar(true);
      return;
    ...
    if (noRulesChange) {
      setResponseMessage({ text: "Cannot proceed when 'No Rules Change' is selected", type: "error" });
      setShowSnackbar(true);
      return;
    }

    const editableRequest = pendingRuleEditRequests.find(
      (req) => req.accountId === selectedSubBroker._id && req.status === 'editable'
    );

    setIsSubmitting(true);

    if (editableRequest) {
      // Update rules during edit window
      ...
      try {
        const response = await TradingRulesService.updateTradingRules({
      ...
      // Request a new edit
      if (pendingRuleEditRequests.some(req => req.accountId === selectedSubBroker._id && req.status === 'pending')) {
        setResponseMessage({ text: "A pending rule edit request already exists for this account", type: "error" });
        setShowSnackbar(true);
        setIsSubmitting(false);
        return;
      }
      ...
      try {
        const response = await TradingRulesService.requestRuleEdit({
          _id: selectedSubBroker._id,
          marketTypeId: selectedSubBroker.marketTypeId,
        });

        if (response.statusCode === 200 && response.success) {
          setResponseMessage({ text: "Rule edit requested successfully. Edit window opens in 4 days.", type: "success" });
          setShowSnackbar(true);
          setTradingRules({ cash: [], future: [], option: [] });
          setSelectedSubBroker(null);
          setSubApiKey('');
          ...
          setResponseMessage({ text: response.message || "Failed to request rule edit", type: "error" });
          setShowSnackbar(true);
        }
      } catch (error: any) {
        console.error("Error requesting rule edit:", error);
        setResponseMessage({ text: error.message || "Failed to request rule edit", type: "error" });
        setShowSnackbar(true);
      } finally {
        setIsSubmitting(false);
      }
  ...
  const renderRuleInput = (rule: Rule, subType: 'cash' | 'future' | 'option') => {
    ...
    const isEditable = selectedSubBroker && pendingRuleEditRequests.some(
      (req) => req.accountId === selectedSubBroker._id && req.status === 'editable'
    );
          ...
          return (
            <Box sx={{ minHeight: 40 }}>
              <Checkbox
                ...
                disabled={!selectedSubBroker || !isMainApiVerified || !isSubApiVerified || !isEditable}
                color="primary"
    ...
    return (
      <Box
        onClick={(!selectedSubBroker || !isMainApiVerified || !isSubApiVerified || !isEditable) ? handleRuleInputClick : undefined}
        sx={{ cursor: (!selectedSubBroker || !isMainApiVerified || !isSubApiVerified || !isEditable) ? 'not-allowed' : 'auto' }}
      >
        {input}
      </Box>
    );
          ...
          <Card sx={{ boxShadow: 2, borderRadius: 2, bgcolor: theme.palette.background.paper }}>
            <CardContent>
              <Typography variant="h5" fontWeight={600} mb={2}>
                Pending Rule Edit Requests (Total: {pendingRuleEditRequests.length})
              </Typography>
              {isLoadingPending ? (
                <Box sx={{ display: "flex", justifyContent: "center", py: 2 }}>
                  <CircularProgress size={24} />
                </Box>
              ) : pendingRuleEditRequests.length > 0 ? (
                <List
                  sx={{
                    maxHeight: 200,
                    overflowY: "auto",
                ...
                >
                  {pendingRuleEditRequests.map((request) => (
                    <ListItem
                      key={request._id}
                    ...
                    >
                      <ListItemText
                        ...
                        secondary={
                          <Typography variant="body2" color={theme.palette.text.secondary}>
                            <strong>Market:</strong> {getMarketTypeDisplayName(request.marketTypeId)} |{' '}
                            ...
                            <strong>Edit From:</strong>{" "}
                            {new Date(request.scheduledEditDate).toLocaleDateString("en-US", {
                              year: "numeric",
                              month: "short",
                              day: "numeric",
                            })} |{' '}
                            <strong>Until:</strong>{" "}
                            {new Date(request.editWindowEndDate).toLocaleDateString("en-US", {
                              year: "numeric",
                              month: "short",
                              day: "numeric",
                            })} |{' '}
                      ...
                      <Button
                        variant="outlined"
                        color="secondary"
                        onClick={() => handleCancelRuleEditRequest(request._id)}
              ...
              <CardContent>
                ...
                <Box sx={{ display: 'flex', justifyContent: 'flex-end', mb: 2 }}>
                  <Button
                    ...
                    disabled={!selectedSubBroker || !isMainApiVerified || !isSubApiVerified || !pendingRuleEditRequests.some(
                      (req) => req.accountId === selectedSubBroker._id && req.status === 'editable'
                    )}
                    sx={{
                      textTransform: 'none',
            ...
            <CardContent>
              ...
              <Box sx={{ mt: 2 }}>
                <Typography variant="body1" color="text.secondary" mb={1}>
                  By requesting a rule edit, you acknowledge:
                </Typography>
                <Typography variant="body2" color="text.primary">
                  • A 2-day edit window will open 4 days after the request.
                </Typography>
                <Typography variant="body2" color="text.primary">
                  • Rules must be edited within the 2-day window, or the request will be canceled.
                </Typography>
                <Typography variant="body2" color="text.primary">
                  • No new edit requests can be made for this account until the current request is completed or canceled.
                </Typography>
              </Box>
            </CardContent>
          </Card>
            ...
            <CardContent>
              ...
              >
                ...
                <Button
                  ...
                  variant="contained"
                  color="primary"
                  disabled={isSubmitting || !selectedSubBroker || !isMainApiVerified || !isSubApiVerified || !termsAccepted || noRulesChange}
                  sx={{
                    maxWidth: { sm: 300 },
                    textTransform: 'none',
                    fontWeight: 600,
                ...
                >
                  {isSubmitting ? (
                    <>
                      ...
                      <Box ml={1}>
                        {pendingRuleEditRequests.some(
                          (req) => req.accountId === selectedSubBroker?._id && req.status === 'editable'
                        ) ? 'Updating Rules...' : 'Requesting Edit...'}
                      </Box>
                    </>
                  ) : (
                    pendingRuleEditRequests.some(
                      (req) => req.accountId === selectedSubBroker?._id && req.status === 'editable'
                    ) ? 'Update Trading Rules' : 'Request Rule Edit'
                  )}
                </Button>
              </Box>
            </CardContent>
